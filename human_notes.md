...
Prototype live URL: https://briocare.onrender.com
...
Access notes / credentials: This demo is a snapshot of the group exercise scenario - where I started by focusing on the therapist UX but later shifted to having the agent run the session. Some tips: as a therapist you will always speak first to establish authority and provide context to the children, then tap the "hand to Brio" button to invite the agent into the session. Multi-party facilitation is a hard problem! I started with a less flexible agentic workflow design to ensure guardrails, but later realized that we need a more flexible, human-like design. Currently, our actor is a single-shot classifier over a fixed menu returning one {action, target, utterance}. It can't say "acknowledge Ana, then open a group question" as one move. One cool direction to explore is arxiv.org/html/2606.13544v2. In addition, I'm not happy with the latency from our STT-LLM-TTS loop. STS with smart guardrails / system 1&2 might help. So, expect some rough edges in agent<>human communication. One alternative if we need to ship fast: creative reduction of agent capability with gamified systems / protocols that kids enjoy. Got a few ideas but more on it later. Otherwise, see README>Known limitations for more info.
